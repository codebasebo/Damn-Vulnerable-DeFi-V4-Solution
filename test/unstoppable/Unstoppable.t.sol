...
contract UnstoppableChallenge is Test {
    ...
    /**
    * My Own Test
    * Check if the vault is paused and the ownership is transferred to deployer
    * Check if the token balance of the vault is equal to the initial token balance
    * Check if the token balance of the player is equal to the initial token balance
    * Check if the monitor contract is owned by the deployer
    * Check if the vault is owned by the deployer
    * Check if the vault is not paused
     */
    function test_assertSolved() public {
